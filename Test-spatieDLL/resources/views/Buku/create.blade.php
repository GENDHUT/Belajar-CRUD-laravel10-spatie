            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('buku.store') }}" enctype="multipart/form-data">
                        ...
                        <!-- Pilih Kategori -->
                        <div class="mb-4">
                            <x-input-label for="kategori" :value="__('Kategori Buku')" />
                                @foreach ($kategori as $kat)
                                    <label class="inline-flex items-center space-x-2 border px-3 py-2 rounded cursor-pointer">
                                        <input type="checkbox" name="kategori[]" value="{{ $kat->id }}" class="hidden peer">
                                        <span class="peer-checked:bg-blue-500 peer-checked:text-black px-2 py-1 rounded bg-gray-700">
                                            {{ $kat->nama_kategori }}
                                        </span>
                                    </label>
                                @endforeach
